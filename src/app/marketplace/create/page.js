...
import { useSupportedCountries } from "@/app/hooks/useSupportedCountries";
...
const empty = {
  ...
  domains: "",
  language: "en",
  sellerSubscriptionAmount: "",
  sellerSubscriptionPromoAmount: "",
  individualPerItemfee: "",
  taxRate: "",
  taxName: "",
  creationCode: "",
};
...
export default function CreateMarketplacePage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (data.success)
          setRegions((data.regions || []).filter((r) => r.isActive));
      } catch {}
    })();
  }, []);
  ...
  const inputDisabled =
    "w-full px-3 py-2.5 border border-gray-200 bg-gray-50 rounded-lg text-sm text-gray-500";
  ...
  const handleCountrySelect = (e) => {
    ...
    setForm((f) => ({
      ...
      timezone: country.timezone || f.timezone,
      currencyCode: (country.currency || f.currencyCode || "").toUpperCase(),
      currencySymbol: country.currencySymbol || f.currencySymbol,
    }));
  };
  ...
  const submit = async (e) => {
    ...
    try {
      const body = {
        ...
        sellerSubscriptionAmount:
          form.sellerSubscriptionAmount !== ""
            ? Number(form.sellerSubscriptionAmount)
            : undefined,
        sellerSubscriptionPromoAmount:
          form.sellerSubscriptionPromoAmount !== ""
            ? Number(form.sellerSubscriptionPromoAmount)
            : null,
        individualPerItemfee:
          form.individualPerItemfee !== ""
            ? Number(form.individualPerItemfee)
            : null,
        settings: {
      ...
      if (!res.ok || !data.success)
        throw new Error(data.message || "Failed to create marketplace");
        ...
        >
          <div>
            ...
            {regions.length === 0 && (
              <p className="text-xs text-amber-600 mt-1">
                ...
                <Link href="/region/create" className="underline">
                  Create one first
                </Link>
                .
              </p>
          ...
          <div>
            ...
            {countriesError ? (
              ...
              <select
                ...
                className={`${input} bg-white disabled:opacity-60 disabled:cursor-wait`}
              >
                <option value="">
            ...
            {countriesLoading && (
              <p className="text-xs text-gray-500 mt-1 flex items-center gap-1">
                <Loader2 className="w-3 h-3 animate-spin" /> Fetching supported
                countries…
              </p>
            )}
          </div>
          ...
          <div>
            ...
            <input
              ...
              placeholder="khalifrex.com, sell.khalifrex.com, admin.khalifrex.com"
              required
              className={input}
            />
          </div>
          ...
          <div className="border-t border-gray-100 pt-5">
            <label className={label}>
              <span className="inline-flex items-center gap-1.5">
                <KeyRound size={14} /> Marketplace creation code
              </span>
            </label>
            <input
              type="password"
              value={form.creationCode}
              onChange={set("creationCode")}
              required
              placeholder="Enter the secret creation code"
              className={input}
            />
          </div>
