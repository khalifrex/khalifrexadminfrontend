...
export default function MarketplaceById() {
    ...
    async function load() {
      ...
      try {
        const [mRes, rRes] = await Promise.all([
          fetch(`${BACKEND}/admin/marketplaces/${marketplaceId}`, { credentials: 'include' }),
          fetch(`${BACKEND}/admin/regions`, { credentials: 'include' }),
        ]);
        const mData = await mRes.json();
        const rData = await rRes.json();
        if (!mRes.ok || !mData.success) throw new Error(mData.message || 'Failed to load');
        ...
        if (!ignore) {
          ...
          setForm({
            ...
            sellerSubscriptionPromoAmount: m.sellerSubscriptionPromoAmount ?? '',
  ...
  const save = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND}/admin/marketplaces/${marketplaceId}`, {
        method: 'PUT',
        credentials: 'include',
  ...
  const toggleActive = async () => {
    ...
    try {
      const res = await fetch(`${BACKEND}/admin/marketplaces/${marketplaceId}/toggle-active`, {
        method: 'PATCH',
        credentials: 'include',
      });
      const data = await res.json();
      if (!res.ok || !data.success) throw new Error(data.message || 'Failed to update');
      setMarketplace((prev) => ({ ...prev, isActive: data.marketplace.isActive }));
    } catch (e) {
      setError(e.message);
    } finally {
      setToggling(false);
      ...
      <div className="max-w-2xl mx-auto px-6 py-10">
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <button onClick={toggleActive} disabled={toggling}
            ...
            {toggling ? 'Updating…' : marketplace.isActive ? 'Deactivate' : 'Activate'}
          </button>
        </div>
        <p className="text-xs text-gray-500 mb-6">{marketplace.marketplaceId}</p>

        <form onSubmit={save} className="bg-white border border-gray-200 rounded-xl p-6 space-y-5">
          <div>
            <label className={label}>Region</label>
            <select value={form.regionId} onChange={set('regionId')} className={`${input} bg-white`}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input value={form.code} onChange={set('code')} required maxLength={5} className={input} />
            </div>
            <div>
          ...
          <div>
            ...
            <input value={form.domains} onChange={set('domains')} required className={input} />
          </div>

          <div className="grid grid-cols-3 gap-4">
            <div>
              <label className={label}>Currency code</label>
              <input value={form.currencyCode} onChange={set('currencyCode')} required maxLength={3} className={input} />
            </div>
            <div>
              <label className={label}>Symbol</label>
              <input value={form.currencySymbol} onChange={set('currencySymbol')} required className={input} />
            </div>
            <div>
              <label className={label}>Currency name</label>
              <input value={form.currencyName} onChange={set('currencyName')} className={input} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input value={form.taxName} onChange={set('taxName')} className={input} />
            </div>
          </div>
