export function buildActionHref(action) {
  ...
  switch (actionType) {
    case "category":
      return value ? `/browse/${encodeURIComponent(value)}` : null;
    ...
    case "search": {
      ...
      if (p.minPrice != null && p.minPrice !== "")
        params.set("minPrice", String(p.minPrice));
      if (p.maxPrice != null && p.maxPrice !== "")
        params.set("maxPrice", String(p.maxPrice));
      if (Array.isArray(p.brands) && p.brands.length)
        params.set("brands", p.brands.join(","));
